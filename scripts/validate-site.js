...
const manifest=store.readJson(manifestPath),expectedMonths=new Set(articles.map(a=>String(a.publishedAt).slice(0,7))),seen=new Set();
if(!Array.isArray(manifest.shards)||manifest.shards.length!==expectedMonths.size)throw new Error('Search manifest shard mismatch');
for(const shard of manifest.shards){if(!expectedMonths.has(shard.month)||!shard.file.startsWith('data/search/')||shard.file.includes('..'))throw new Error('Invalid search shard');const value=store.readJson(path.join(root,shard.file));for(const [id,doc] of Object.entries(value.documents||{})){if(seen.has(id))throw new Error('Duplicate search record: '+id);seen.add(id);if(!ids.has(id)||typeof doc.text!=='string')throw new Error('Orphan or malformed search record: '+id);}}
