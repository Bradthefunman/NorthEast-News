...
function validateArticle(article, sourcePath) {
  ...
  for (const flag of ['breaking','developing','analysis','featured','trending','archive']) if (typeof article[flag] !== 'boolean') throw new Error('Article flag must be boolean "' + flag + '": ' + article.id);
...
module.exports = {ROOT,DATA_DIR,ARTICLE_DIR,INDEX_PATH,ID_PATTERN,REQUIRED_FIELDS,stableValue,stableStringify,isValidTimestamp,validateArticleId,validateArticle,validateUnique,listArticleFiles,readJson,loadArticles,validateArticles,parseLegacy,parseTimestamp,effectiveTime,compareArticles,catalogRecord,catalogForArticles,catalogFromDisk,writeAtomic,writeCatalog,findArticleById,findArticleBySlug};
