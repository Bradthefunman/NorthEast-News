(function () {
  ...
  function renderHome() {
    ...
    setContent('nh-grid', articles.filter(function (item) { return stateMatches(item, 'NH') || item.topic === 'new-hampshire' || item.category === 'new-hampshire'; }).slice(0, 6).map(storyCard).join(''));
    setContent('ne-grid', articles.filter(function (item) { return item.category === 'new-england' || item.topic === 'massachusetts' || item.topic === 'rhode-island'; }).slice(0, 6).map(storyCard).join(''));
