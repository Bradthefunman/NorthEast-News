(function () {
  ...
  function render() {
    ...
    var feed = document.getElementById('landing-feed');
    var feedItems = items.slice(lead ? 1 : 0), visibleCount = pageSize;
    var button = document.getElementById('landing-load-more');
    if (!button && feed) {
      button = document.createElement('button');
      button.id = 'landing-load-more';
      button.className = 'load-more-button';
      button.type = 'button';
      button.setAttribute('aria-controls', 'landing-feed');
      button.textContent = 'Load more stories';
      feed.insertAdjacentElement('afterend', button);
    }
    function renderFeed() {
      if (feed) feed.innerHTML = feedItems.slice(0, visibleCount).map(api.latestItem).join('') || '<p class="empty-state">More coverage will appear here as stories are published.</p>';
      if (button) button.hidden = visibleCount >= feedItems.length;
    }
    if (button) {
