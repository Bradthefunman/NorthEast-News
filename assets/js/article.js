(function () {
  ...
  function inlineMarkup(value, site) {
    var html = site.escapeHTML(value);
    ...
    html = html.replace(/(^|[^*])\*([^*]+)\*(?!\*)/g, '$1<em>$2</em>').replace(/(^|[^_])_([^_]+)_(?!_)/g, '$1<em>$2</em>');
    return html;
  }
  function bodyMarkup(body, site) {
    return (body || []).map(function (block) {
      var lines = String(block || '').split(/\r?\n/).map(function (line) { return line.trim(); }).filter(Boolean);
      if (!lines.length) return '';
      if (lines.every(function (line) { return /^[-*]\s+/.test(line); })) return '<ul>' + lines.map(function (line) { return '<li>' + inlineMarkup(line.replace(/^[-*]\s+/, ''), site) + '</li>'; }).join('') + '</ul>';
      if (lines.every(function (line) { return /^\d+[.)]\s+/.test(line); })) return '<ol>' + lines.map(function (line) { return '<li>' + inlineMarkup(line.replace(/^\d+[.)]\s+/, ''), site) + '</li>'; }).join('') + '</ol>';
      if (lines.every(function (line) { return /^>\s?/.test(line); })) return '<blockquote>' + lines.map(function (line) { return inlineMarkup(line.replace(/^>\s?/, ''), site); }).join('<br>') + '</blockquote>';
      if (/^#{1,3}\s+/.test(lines[0])) return '<h3>' + inlineMarkup(lines[0].replace(/^#{1,3}\s+/, ''), site) + '</h3>' + (lines.length > 1 ? '<p>' + lines.slice(1).map(function (line) { return inlineMarkup(line, site); }).join('<br>') + '</p>' : '');
      return '<p>' + lines.map(function (line) { return inlineMarkup(line, site); }).join('<br>') + '</p>';
    }).join('');
  }
  ...
  function render(article, all) {
    ...
    var body = Array.isArray(article.body) ? article.body : [article.body || ''], bodyHtml = bodyMarkup(body, site), relatedItems = related(article, all);
